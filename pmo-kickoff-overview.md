---
name: pmo-kickoff-overview
description: Turn first/kickoff meeting notes with a project requestor into a structured project overview and team roster, then create it as a child subpage under the project's Notion node — following the HostPapa PMO project-initiation methodology. Use whenever the user has notes, a transcript, or a recap from an initial "what is this project" meeting (Gemini notes, Notion notes, pasted text) and wants a project overview, kickoff summary, or Notion "entry point" page from it — e.g. "turn these kickoff notes into an overview," "summarize this intake call for the project page," "draft the overview for the WordPress migration project." Asks for the project node URL and publishes as a subpage there rather than just returning text. First skill in the pmo- family encoding this PMO's process; other pmo-* skills cover later stages (team intake, scheduling, meeting recaps, Slack/Notion sync, project registry).
---

# PMO Kickoff Overview

## Why this exists

The PMO's standard practice for starting a project is a single first meeting with whoever is requesting it. The core of that meeting is a fixed set of intake questions — what the project is, why it matters, what happens if it doesn't get done, plus a few more that round out what a PM actually needs before work starts: how success is judged, when it needs to land, what's already known to be a constraint or risk, and what's still unresolved. The answers become the project's overview — the page anyone lands on to understand the project at a glance — and the same conversation usually surfaces which teams need to be pulled in. The PM doesn't write this by hand; they hand the meeting notes to Claude and ask for the overview directly. This skill is that step, made repeatable.
...
Do not schedule anything — no kickoff calls, no planning sessions, no calendar coordination. That's a separate, not-yet-defined part of the PMO's process. This skill stops at producing the overview and roster; if the user asks you to also line up meetings from the same notes, tell them that's a different (future) skill and isn't part of this one.

Similarly, don't build out the full project workbook (action items, risks, schedule of steps, etc.) — that's the next stage of the methodology and belongs to a different skill once it exists. This one is scoped to the entry-point overview only.
